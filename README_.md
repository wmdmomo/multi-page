...
####  vm.<img src="https://latex.codecogs.com/gif.latex?listeners包含了父作用域中的%20(不含%20``.native``%20修饰器的)%20``v-on``%20事件监听器。它可以通过%20``v-on=&quot;"/>listeners"`` 传入内部组件——在创建更高层次的组件时非常有用。
